...
class VMInterfaceCreateHandler(ResourceCreateHandler, VMInterfaceMixin):
    ...
    def _get_tenant_id_for_create(self, context, resource):
        print('DEBUG CONTEXT GET TENANT ID FOR CREATE', context)
        if context['is_admin'] and 'tenant_id' in resource:
            tenant_id = resource['tenant_id']
        elif ('tenant_id' in resource and 'tenant' in context and
              resource['tenant_id'] != context['tenant']):
            self._raise_contrail_exception('AdminRequired',
            ...
            self._raise_contrail_exception('AdminRequired',
                reason='Cannot create resource if tenant is not specified',
                resource='port')
        return tenant_id
